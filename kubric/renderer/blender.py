...
def _first_existing_input(inputs, *names):
    for name in names:
        socket = inputs.get(name)
        if socket is not None:
            return socket
    raise KeyError(f"None of these sockets were found: {names!r}")


def _color_to_factor(value):
...
class Blender(core.View):
    ...
    def __init__(
        ...
        self.use_gpu = os.getenv("KUBRIC_USE_GPU", "False").lower() in (
            "true",
            "1",
            "t",
        )
        ...
        super().__init__(
            ...
            scene_observers={
                ...
                "ambient_illumination": [
                    lambda change: self._set_ambient_light_color(change.new)
                ],
    ...
    def postprocess(self, from_dir: PathLike, return_layers: Sequence[str]):
        ...
        png_frames = [
            from_dir / "images" / (exr_filename.stem + ".png")
        ...
        return {key: np.stack(data_stack[key], axis=0) for key in data_stack}
    ...
    def _add_asset(self, asset: core.Cube):
        ...
        asset.observe(
            AttributeSetter(
                cube, "active_material", converter=self._convert_to_blender_object
            ),
            "material",
        )
    ...
    def _add_asset(self, obj: core.Sphere):
        ...
        obj.observe(
            AttributeSetter(
                sphere, "active_material", converter=self._convert_to_blender_object
            ),
            "material",
        )
    ...
    def _add_asset(self, obj: core.FileBasedObject):
        ...
        with RedirectStream(
            stream=sys.stdout, disabled=self.verbose
                ...
                with redirect_stdout(fstdout):  # < also suppresses python stdout
                    if extension == "obj":
                        ...
                        bpy.ops.import_scene.gltf(
                            filepath=obj.render_filename, **obj.render_import_kwargs
                        )
                        ...
                        if obj.use_parenting_instead_of_join:
                            ...
                            with bpy.context.temp_override(
                                selected_objects=non_mesh_objects
                            ):
    ...
    @blender_utils.prepare_blender_object
    def _add_asset(
        self, obj: core.DirectionalLight
    ...
    def _add_asset(self, obj: core.SpotLight):  # pylint: disable=function-redefined
        ...
        obj.observe(
            KeyframeSetter(spotlight, "spot_blend"), "spot_blend", type="keyframe"
        )
        obj.observe(AttributeSetter(spotlight, "spot_size"), "spot_size")
        obj.observe(
            KeyframeSetter(spotlight, "spot_size"), "spot_size", type="keyframe"
        )
    ...
    def _add_asset(self, obj: core.PerspectiveCamera):
        ...
        obj.observe(
            KeyframeSetter(camera, "sensor_width"), "sensor_width", type="keyframe"
        )
        obj.observe(AttributeSetter(camera, "clip_start"), "min_render_distance")
        obj.observe(
            KeyframeSetter(camera, "clip_start"), "min_render_distance", type="keyframe"
        )
        obj.observe(AttributeSetter(camera, "clip_end"), "max_render_distance")
        obj.observe(
            KeyframeSetter(camera, "clip_end"), "max_render_distance", type="keyframe"
        )
    ...
    def _add_asset(self, obj: core.OrthographicCamera):
        ...
        obj.observe(
            KeyframeSetter(camera, "ortho_scale"), "orthographic_scale", type="keyframe"
        )
        obj.observe(AttributeSetter(camera, "clip_start"), "min_render_distance")
        obj.observe(
            KeyframeSetter(camera, "clip_start"), "min_render_distance", type="keyframe"
    ...
    def _add_asset(self, obj: core.PrincipledBSDFMaterial):
        ...
        obj.observe(
            AttributeSetter(bsdf_node.inputs["Base Color"], "default_value"), "color"
        )
        obj.observe(
            KeyframeSetter(bsdf_node.inputs["Base Color"], "default_value"),
            "color",
            type="keyframe",
        )
        obj.observe(
            AttributeSetter(bsdf_node.inputs["Roughness"], "default_value"), "roughness"
        )
        obj.observe(
            KeyframeSetter(bsdf_node.inputs["Roughness"], "default_value"),
            "roughness",
            type="keyframe",
        )
        obj.observe(
            AttributeSetter(bsdf_node.inputs["Metallic"], "default_value"), "metallic"
        )
        obj.observe(
            KeyframeSetter(bsdf_node.inputs["Metallic"], "default_value"),
            "metallic",
            type="keyframe",
        )
        specular_input = _first_existing_input(
            bsdf_node.inputs, "Specular IOR Level", "Specular"
        )
        obj.observe(AttributeSetter(specular_input, "default_value"), "specular")
        obj.observe(
            KeyframeSetter(specular_input, "default_value"), "specular", type="keyframe"
        )
        specular_tint_input = bsdf_node.inputs["Specular Tint"]
        specular_tint_converter = None
        if specular_tint_input.bl_idname != "NodeSocketColor":
            specular_tint_converter = _color_to_factor
        obj.observe(
            AttributeSetter(
                specular_tint_input, "default_value", converter=specular_tint_converter
            ),
            "specular_tint",
        )
        obj.observe(
            KeyframeSetter(specular_tint_input, "default_value"),
            "specular_tint",
            type="keyframe",
        )
        obj.observe(AttributeSetter(bsdf_node.inputs["IOR"], "default_value"), "ior")
        obj.observe(
            KeyframeSetter(bsdf_node.inputs["IOR"], "default_value"),
            "ior",
            type="keyframe",
        )
        transmission_input = _first_existing_input(
            bsdf_node.inputs, "Transmission Weight", "Transmission"
        )
        obj.observe(
            AttributeSetter(transmission_input, "default_value"), "transmission"
        )
        obj.observe(
            KeyframeSetter(transmission_input, "default_value"),
            "transmission",
            type="keyframe",
        )
        emission_input = _first_existing_input(
            bsdf_node.inputs, "Emission Color", "Emission"
        )
        obj.observe(AttributeSetter(emission_input, "default_value"), "emission")
        obj.observe(
            KeyframeSetter(emission_input, "default_value"), "emission", type="keyframe"
        )
        return mat
    ...
    def _add_asset(self, obj: core.FlatMaterial):
        ...
        tree.links.new(
            light_path_node.outputs["Is Camera Ray"], overall_mix_node.inputs["Fac"]
        )
        ...
        tree.links.new(
            overall_mix_node.outputs["Shader"], output_node.inputs["Surface"]
        )

        obj.observe(
            AttributeSetter(emission_node.inputs["Color"], "default_value"), "color"
        )
        obj.observe(
            KeyframeSetter(emission_node.inputs["Color"], "default_value"),
            "color",
            type="keyframe",
        )
        obj.observe(
            AttributeSetter(holdout_mix_node.inputs["Fac"], "default_value"), "holdout"
        )
        obj.observe(
            KeyframeSetter(holdout_mix_node.inputs["Fac"], "default_value"),
            "holdout",
            type="keyframe",
        )
        obj.observe(
            AttributeSetter(indirect_mix_node.inputs["Fac"], "default_value"),
            "indirect_visibility",
        )
        obj.observe(
            KeyframeSetter(indirect_mix_node.inputs["Fac"], "default_value"),
            "indirect_visibility",
            type="keyframe",
        )
        return mat
    ...
    def _setup_scene_shading(self):
        ...
        self.ambient_node.inputs["Color"].default_value = (0.0, 0.0, 0.0, 1.0)
        self.ambient_node.location = 700, 0
        self.bg_node = tree.nodes.new(type="ShaderNodeBackground")
        self.bg_node.inputs["Color"].default_value = (0.0, 0.0, 0.0, 1.0)
        self.bg_node.location = 700, -120

        links.new(
            lightpath_node.outputs.get("Is Camera Ray"), mix_node.inputs.get("Fac")
        )
        ...
        links.new(
            coord_node.outputs.get("Generated"),
            self.bg_mapping_node.inputs.get("Vector"),
        )
        links.new(
            self.bg_mapping_node.outputs.get("Vector"),
            self.bg_hdri_node.inputs.get("Vector"),
        )
        ...
        links.new(
            coord_node.outputs.get("Generated"),
            self.illum_mapping_node.inputs.get("Vector"),
        )
        links.new(
            self.illum_mapping_node.outputs.get("Vector"),
            self.ambient_hdri_node.inputs.get("Vector"),
        )

    def _set_ambient_light_color(self, color=(0.0, 0.0, 0.0, 1.0)):
    ...
    def _set_ambient_light_hdri(
        self, hdri_filepath=None, hdri_rotation=(0.0, 0.0, 0.0), strength=1.0
    ):
        # ensure hdri_node is connected
        self.blender_scene.world.node_tree.links.new(
            self.ambient_hdri_node.outputs.get("Color"),
            self.ambient_node.inputs.get("Color"),
        )
        self.ambient_hdri_node.image = bpy.data.images.load(
            hdri_filepath, check_existing=True
        )
    ...
    def _set_background_hdri(self, hdri_filepath=None, hdri_rotation=(0.0, 0.0, 0.0)):
        # ensure hdri_node is connected
        self.blender_scene.world.node_tree.links.new(
            self.bg_hdri_node.outputs.get("Color"), self.bg_node.inputs.get("Color")
        )
        self.bg_hdri_node.image = bpy.data.images.load(
            hdri_filepath, check_existing=True
        )
...
def register_object3d_setters(obj, blender_obj):
    ...
    obj.observe(
        KeyframeSetter(blender_obj, "rotation_quaternion"),
        "quaternion",
        type="keyframe",
    )
