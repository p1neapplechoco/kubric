...
def set_up_exr_output_node(
    ...
    for layer_name in aux_layers:
        out_node.file_slots.new(layer_name)
        links.new(
            render_node_aux.outputs.get(layer_name), out_node.inputs.get(layer_name)
        )
    ...
    if motion_blur is not None:
        ...
        links.new(
            render_node.outputs.get("Image"), motion_blur_node.inputs.get("Image")
        )
        links.new(render_node.outputs.get("Depth"), motion_blur_node.inputs.get("Z"))
        links.new(
            render_node_aux.outputs.get("Vector"), motion_blur_node.inputs.get("Speed")
        )
        links.remove(out_node.inputs.get("Image").links[0])
        links.new(motion_blur_node.outputs.get("Image"), out_node.inputs.get("Image"))
        links.new(
            motion_blur_node.outputs.get("Image"), composite_out.inputs.get("Image")
        )
...
def add_coordinate_material():
    ...
    mat.node_tree.links.new(
        tex_coordinates.outputs.get("Generated"), aov_out_node.inputs.get("Color")
    )
    mat.node_tree.links.new(
        tex_coordinates.outputs.get("Generated"),
...
def read_channels_from_exr(
    exr: OpenEXR.InputFile, channel_names: Sequence[str]
) -> np.ndarray:
...
def get_render_layers_from_exr(filename) -> Dict[str, np.ndarray]:
    ...
    if "Image" in layer_names:
        ...
        output["linear_rgba"] = read_channels_from_exr(
    ...
    if "Normal" in layer_names:
        # range: [-1, 1]
        output["normal"] = read_channels_from_exr(
    ...
    if "ObjectCoordinates" in layer_names:
        output["object_coordinates"] = read_channels_from_exr(
            exr, ["ObjectCoordinates.R", "ObjectCoordinates.G", "ObjectCoordinates.B"]
        )
    return output


def replace_cryptomatte_hashes_by_asset_index(
    segmentation_ids: ArrayLike, assets: Sequence[core.assets.Asset]
):
...
def apply_transformations(
    ...
    with selected(objects):
        bpy.ops.object.transform_apply(
            location=position, rotation=rotation, scale=scale
        )
...
def get_vertices_and_faces(obj: bpy.types.Object) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if not isinstance(obj.data, bpy.types.Mesh):
        raise ValueError(
            f"Expected mesh object, but got {obj.name!r} which is {obj.type!r}"
        )
    bmesh = obj.data
...
def process_normal(exr_layers, scene):  # pylint: disable=unused-argument
    ...
    return ((exr_layers["normal"].clip(-1.0, 1.0) + 1) * 65535 / 2).astype(np.uint16)
...
def process_object_coordinates(exr_layers, scene):  # pylint: disable=unused-argument
    ...
    return (exr_layers["object_coordinates"].clip(0.0, 1.0) * 65535).astype(np.uint16)
